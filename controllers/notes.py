...
people data. Works like the people.py module, except it must handle both a person_id and a note_id
"""
...
def read_all_mine(person_id):
    """
    This function responds to a request for
    /api/people/{person_id}/notes
    with all notes for the associated person
    ...
    if note_list is not None:
        note_schema = NoteSchema(many=True, exclude=["person"])
        data = note_schema.dump(note_list)
        return data

    # Otherwise, nope, didn't find that note
    else:
        abort(404, f"Notes not found for Id: {person_id}")
